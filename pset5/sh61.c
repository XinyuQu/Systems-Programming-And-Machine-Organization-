...
static command* command_alloc(void) {
    ...
    c->infile = c->outfile = c->errfile = c->cd_dir = NULL;
...
pid_t start_command(command* c, pid_t pgid) {
    ...
    if (c->has_pipe == 1){
        ...
        if(fork_return == -1){
            ...
            if(c->in_fd != 0){
                ...
                close(c->in_fd);
                    ...
                    if (c-> outfile != NULL){
                        fd = open(c->outfile,O_WRONLY | O_CREAT | O_TRUNC, 0666);
                        if(fd == -1){
                            fprintf(stderr, "No such file or directory\n");
                            _exit(1);
                        }
                        else{
                        int child = dup2(fd, STDOUT_FILENO);
                        if(child == -1){
                            _exit(1);
                        }
                        close(fd);
                    ...
                    if (c->errfile != NULL){
                        fd = open(c->errfile,O_WRONLY|O_CREAT|O_TRUNC, 0666);
                         if(fd == -1){
                            fprintf(stderr, "No such file or directory\n");
                            _exit(1);
                        }
                        else{           
                         int child = dup2(fd, STDERR_FILENO);
                         if(child == -1){
                            _exit(1);
                        }
                         close(fd);
        ...
        int id = setpgid(fork_return, current_group_id);
        pgid = current_group_id;
        if(id == -1)
     ...
     //book-keeping   
        c->pid = fork_return;
        if(c->has_pipe == 1){
            set_foreground(current_group_id);
...
//    and write code in runlist (or in helper functions!).
   ...
   struct command* head2 = c;
    while ((s = parse_shell_token(s, &type, &token)) != NULL) {
        ...
        if (type == TOKEN_REDIRECTION){
            ...
            while(type == TOKEN_REDIRECTION || type == TOKEN_NORMAL)
            {
                char* tok = token;
...
int main(int argc, char* argv[]) {
    ...
    while (!feof(command_file)) {
       ...
       handle_signal(SIGINT, inter_handler);
        //set_foreground(0);
       /* printf("sh61[%d]$ ", getpid());
        fflush(stdout);
        needprompt = 0;
